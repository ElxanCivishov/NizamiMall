...
const BlogCard = ({ blog }) => {
  return (
    blog && (
      <div className="w-full h-full bg-white rounded-lg transition-all duration-200  group hover:-translate-y-3">
        <Link to={`/xeber/${blog.id}`} className="h-full ">
          <div className="w-full h-full flex items-center justify-center   max-h-[300px] bg-white">
            <img
              src={blog.image || noimage}
              alt=""
              className="w-full m-5 h-full group-hover:scale-105 transition-all duration-200  object-cover "
            />
          </div>
